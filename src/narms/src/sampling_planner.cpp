...
int main(int argc, char*argv[])
{
	...
	ros::ServiceClient move_pr2_to_pose = nh.serviceClient<narms::target_pose>("pr2/move_pr2_to_pose");
	...
	std::string pr2_planning_group("right_arm");
	...
	while(!handoff_sucess)
	{
		...
		if(pr2_compute_ik.call(pr2_ik_srv))
		{
			...
			if(pr2_ik_srv.response.error_code.val == 1)
			{
				...
				narms::target_pose move_msg ;
				move_msg.request.pose = sampled_handoff_pose;
				bool suc = move_pr2_to_pose.call(move_msg);
				std::cout << "PR2 move to pose Service result :" << suc << "\n";
			}
		}else{
